...
func New(ctx context.Context, h host.Host, permissionLayer permLayer.PermissionLayer, options ...opts.Option) (*IpfsDHT, error) {
	var cfg opts.Options
	cfg.BucketSize = KValue
	if err := cfg.Apply(append([]opts.Option{opts.Defaults}, options...)...); err != nil {
		return nil, err
	}
	dht := makeDHT(ctx, h, cfg.Datastore, cfg.Protocols, cfg.BucketSize, permissionLayer)
...
func NewDHT(ctx context.Context, h host.Host, dstore ds.Batching, permissionLayer permLayer.PermissionLayer) *IpfsDHT {
	dht, err := New(ctx, h, permissionLayer, opts.Datastore(dstore))
	if err != nil {
		panic(err)
	}
	return dht
...
func NewDHTClient(ctx context.Context, h host.Host, dstore ds.Batching, permissionLayer permLayer.PermissionLayer) *IpfsDHT {
	dht, err := New(ctx, h, permissionLayer, opts.Datastore(dstore), opts.Client(true))
	if err != nil {
		panic(err)
	}
	return dht
...
func makeDHT(ctx context.Context, h host.Host, dstore ds.Batching, protocols []protocol.ID, bucketSize int, permissionLayer permLayer.PermissionLayer) *IpfsDHT {
