...
func (l *listener) Accept() (transport.CapableConn, error) {
	for c := range l.incoming {
		...
		if !c.IsClosed() {
			if l.PermissionLayer.ID() != "NON" {
				peerIsValidated := false
				to := time.Duration(2000) * time.Millisecond
				validated, _ := l.PermissionLayer.ValidatePeer(c.RemotePeer(), to) // w timeout
				peerIsValidated = validated
				if peerIsValidated {
					go func(ll *listener, cc transport.CapableConn) {
						ll.PermissionLayer.UpdateIndividualSPProfileCache(cc.RemotePeer())
					}(l, c)
					return c, nil
				} else {
					c.Close()
					l.err = fmt.Errorf("error Accept: peer is not valid wrt permissionLayer")
					return nil, l.err
				}
			} else {
