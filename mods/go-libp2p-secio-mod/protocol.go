...
func (sg *Transport) newSecureSession(ctx context.Context, local peer.ID, key ci.PrivKey, insecure net.Conn, remotePeer peer.ID) (*secureSession, error) {
	s := &secureSession{localPeer: local, localKey: key, PermissionLayer: sg.PermissionLayer}
...
func (s *secureSession) runHandshake(ctx context.Context) error {
	fmt.Println("DEBUG RUN HANDSHAKE")
...
func (s *secureSession) runHandshakeSync() error {
	...
	case "":
		...
		if s.PermissionLayer.ID() != "NON" {
			peerIsValidated := false
			to := time.Duration(2000) * time.Millisecond
			validated, _ := s.PermissionLayer.ValidatePeer(actualRemotePeer, to) // w timeout
			peerIsValidated = validated
			if peerIsValidated {
				go func(ss *secureSession, p peer.ID) {
					ss.PermissionLayer.UpdateIndividualSPProfileCache(p)
				}(s, actualRemotePeer)
			} else {
				s.insecure.Close()
				log.Debugf("peer %s is not registered with %s", actualRemotePeer, string(s.PermissionLayer.ID()))
			}
		}
