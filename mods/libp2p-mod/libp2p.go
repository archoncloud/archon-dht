...
import (
	...
	config "github.com/archoncloud/archon-dht/mods/libp2p-mod/config"
...
func NewPermissioned(ctx context.Context, permissionLayer permLayer.PermissionLayer, opts ...Option) (host.Host, error) {
	return NewPermissionedWithoutDefaults(ctx, permissionLayer, append(opts, FallbackDefaults)...)
}
...
func NewPermissionedWithoutDefaults(ctx context.Context, permissionLayer permLayer.PermissionLayer, opts ...Option) (host.Host, error) {
	var cfg Config
	if err := cfg.Apply(opts...); err != nil {
		return nil, err
	}
	cfg.PermissionLayer = permissionLayer
	return cfg.NewNode(ctx)
}
