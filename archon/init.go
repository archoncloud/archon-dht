...
import (
	...
	archonCommon "github.com/archoncloud/archon-dht/common"
...
// Init is essentially the main entrypoint to archon-dht, see README
// for suggestions on populating and collating configArr and setting basePort
// For each config in configArr, initializes a dht layer corresponding to
// respective permissionLayer of config
func Init(configArr []DHTConnectionConfig, basePort int) (*ArchonDHTs, error) {
	...
	/*go func() {
		for { // this is immensely useful for deep debugging
			fmt.Println("peers")
